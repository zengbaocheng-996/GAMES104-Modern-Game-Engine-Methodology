...
namespace Piccolo
{
                              ...
                              void*&                        target_instance)
    {
        ...
        if(component_iter!=components.end())
        {
            ...
            while(std::getline(iss, current_name, '.')) // find target field
            {
                ...
                field_instance = field_accessor.get(target_instance); // for next iteration
    ...
    static void LuaComponent::set(std::weak_ptr<GObject> game_object, const char* name, T value)
    {
        ...
        {
            LOG_ERROR("Can't find SET target field");
        };
    }
    ...
    static T LuaComponent::get(std::weak_ptr<GObject> game_object, const char* name)
    {
        ...
        {
            LOG_ERROR("Can't find GET target field.");
        }
    }
